...
int _modbus_receive_msg(modbus_t *ctx, uint8_t *msg, msg_type_t msg_type)
{
    ...
    if(msg_type == MSG_INDICATION){
	  timeout=ctx->indication_timeout;
    }else {
    ...
    case MODBUS_FC_WRITE_MULTIPLE_REGISTERS: {
        ...
        if (nb < 1 || MODBUS_MAX_WRITE_REGISTERS < nb) {
            ...
            rsp_length = response_exception(
                ...
                mapping_address < 0 ? address : address + nb);
        } else if (modbus_write_registers_check_callback(ctx,mapping_address,&req[offset + 6],nb) != 0) {
            rsp_length = response_exception(
                ctx, &sft, MODBUS_EXCEPTION_ILLEGAL_DATA_VALUE, rsp, FALSE);
            log_debug("Illegal data value in write_registers start addr:%d. cnt:%d.\r\n", mapping_address,nb);
        }else {
            int i, j;
...
int modbus_set_response_timeout(modbus_t *ctx, uint32_t timeout)
{
    if (ctx == NULL ||
        timeout == 0) {
...
int modbus_set_byte_timeout(modbus_t *ctx, uint32_t timeout)
{
...
int modbus_set_indication_timeout(modbus_t *ctx, uint32_t timeout)
{
...
__weak int modbus_write_registers_check_callback(modbus_t *ctx,int addr_offset,const uint8_t *value,int cnt)
{
