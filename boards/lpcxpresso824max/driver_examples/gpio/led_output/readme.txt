...
When the GPIO feature is enabled, initialize the port and the pin connected to LED and SW1, configure the port mask with MASK register
and MPIN register, which can be used to monitor the every GPIO's status. Green LED will blink when users hold the SW1 button.

## Functional Description
-------------------------
In this example, The GPIO will output the status into serial terminal. 

When start running this example, main routine will initialize clock, pin mux configuration, and configure the GPIO module to make it work in interrupt way. Green LED will blink when users hold the SW1 button.
...
## Expected Result
------------------------
Green LED will blink when users hold the SW1 button.
